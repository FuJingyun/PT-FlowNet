...
from tools.test import test_part
from tools.val import val_part
...
# from tools.test_refine import test_part_refine
from tools.val_refine import val_part_refine
from tools.train_refine import train_refine
...
def log_init(args, mode='Train'):
    ...
    if not os.path.exists(log_dir):
        os.mkdir(log_dir)
    log_name = mode + '_' + args.dataset + '.log'
    logging.basicConfig(
    ...
    ckpt_dir = os.path.join(args.exp_path, 'checkpoints')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.dataset == 'FT3D':
        ...
        train_dataset = FT3D(root_dir=dataset_path, nb_points=args.max_points, mode='train')
        val_dataset = FT3D(root_dir=dataset_path, nb_points=args.max_points, mode='val')
    ...
    if main_process():     
        logger.info("train_data samples: '{}'".format(len(train_dataset)))
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None),
                                num_workers=args.workers, pin_memory=True, sampler=train_sampler,collate_fn=Batch, drop_last=True)
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size_val, shuffle=False,
                                num_workers=args.workers, pin_memory=True, sampler=val_sampler,collate_fn=Batch)
    ...
    lr_scheduler = CosineAnnealingLR(optimizer, T_max=args.num_epochs*len(train_dataset)) 
    

    if args.begin_epoch > 1:
        for _ in range(args.begin_epoch-1):
            lr_scheduler.step()
        args.num_epochs += args.begin_epoch-1
    ...
    for epoch in range(args.begin_epoch, args.num_epochs+1):
        ...
        if not args.refine:
            loss_train, epe_train = train(args,epoch,model,train_dataloader,optimizer)
        else:
            loss_train, epe_train = train_refine(args,epoch,model,train_dataloader,optimizer)   
        ...
        if main_process():
            ...
            writer.add_scalar('mIoU_train', epe_train, epoch_log)
            
        is_best = False
        
        if not args.refine:
            loss_meter, epe_meter,outlier_meter,acc3dRelax_meter,acc3dStrict_meter = val_part(args, model,val_dataloader, epoch=0)
        else:
            loss_meter, epe_meter,outlier_meter,acc3dRelax_meter,acc3dStrict_meter = val_part_refine(args, model,val_dataloader, epoch=0)
            
        if main_process():
            writer.add_scalar('loss_val', loss_meter, epoch_log)
            writer.add_scalar('epe_val', epe_meter, epoch_log)
            writer.add_scalar('outlier_val',outlier_meter, epoch_log)
            writer.add_scalar('acc3dRelax_val', acc3dRelax_meter, epoch_log)
            writer.add_scalar('acc3dStrict_val', acc3dStrict_meter, epoch_log)
            is_best = epe_meter < best_epe
            best_epe = min(best_epe, epe_meter)
            
        if (epoch_log % args.save_freq == 0) and main_process():
            filename = save_checkpoint(model, args, epoch, 'train')
            ...
            if is_best:
                logger.info('Best best_epe updated to: {:.4f}'.format(best_epe))
                
                ckpt_dir = os.path.join(args.exp_path, 'checkpoints')
                checkpoint_name = f'{epoch:03d}.params'
                checkpoint_path = os.path.join(ckpt_dir, checkpoint_name)
                myname = 'best_checkpoint.params'
                shutil.copyfile(checkpoint_path, os.path.join(ckpt_dir, myname))        
    
    if main_process():
        writer.close()
        logger.info('==>Training done!\nBest EPE: %.3f' % (best_epe)) 
    
    # load weight
    weight_path = os.path.join(args.exp_path, 'checkpoints','best_checkpoint.params') 
...
if __name__ == '__main__':
    ...
    gc.collect()
    args = parse_args()
    args.train_gpu = args.gpus.split(',')
    main(args)
