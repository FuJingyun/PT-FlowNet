...
def train(args, epoch,model,train_dataloader,optimizer):
    ...
    for i, batch_data in enumerate(train_progress):
        ...
        optimizer.zero_grad()
        batch_data_input = batch_data
            
        est_flow = model(batch_data_input["sequence"], num_iters = args.iters)
             
        loss = sequence_loss(est_flow, batch_data_input, gamma=args.gamma)
        epe = compute_epe_train(est_flow[-1], batch_data_input)
