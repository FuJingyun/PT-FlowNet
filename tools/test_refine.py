...
def test_part_refine(args, model,test_dataloader, epoch=0):
    ...
    for i, batch_data in enumerate(run_progress):
        ...
        if (i + 1) % args.print_freq == 0 and main_process(args):
            logger.info('Test: [{}/{}] '
                        ...
                        'acc3dStrict{acc3dStrict_meter.val:.4f}'.format(i + 1, len(test_dataloader),
                                                          data_time=data_time,
    ...
    if main_process(args):
        logger.info('Test result: LOSS/EPE/Outlier/acc3dRelax/acc3dStrict {:.4f}/{:.4f}/{:.4f}/{:.4f}/{:.4f}.'.format(loss_meter.avg, epe_meter.avg,outlier_meter.avg,acc3dRelax_meter.avg,acc3dStrict_meter.avg))
        logger.info('<<<<<<<<<<<<<<<<< End Test <<<<<<<<<<<<<<<<<')
    
    if args.use_all_training_data :
