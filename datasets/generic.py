...
class Batch:
    ...
    def myto(self):
        ...
        for key in self.data.keys():
            self.data[key] = [d.cuda(non_blocking=True) for d in self.data[key]]

        return self
    
    def tocuda(self, *args, **kwargs):
...
class SceneFlowDataset(Dataset):
    ...
    def subsample_points(self, sequence, ground_truth):
        ...
        """
        if sequence[0].shape[0]>self.nb_points and sequence[1].shape[0]>self.nb_points:
