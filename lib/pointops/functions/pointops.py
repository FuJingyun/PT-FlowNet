...
def queryandgroup_refine(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True):
    """
    input: xyz: (n, 3), new_xyz: (m, 3), feat: (n, c), idx: (m, nsample), offset: (b), new_offset: (b)
    output: new_feat: (m, c+3, nsample), grouped_idx: (m, nsample)
    """
    ...
    # 用相对的特征与原PV-RAFT refine模块思路相同,注意最后输出的流应加上当前估计的部分，PT只提供残差
    grouped_feat -= feat.unsqueeze(1)
    #grouped_feat = grouping(feat, idx) # (m, nsample, c)
